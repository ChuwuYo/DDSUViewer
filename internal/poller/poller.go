// internal/poller/poller.go: 轮询器（Poller）。
// 说明：负责按配置构建周期性测量任务（Modbus 请求帧），调度串口读写、处理超时/重试与指数退避，并将原始寄存器数据发送到上层数据通道供 Service 解析使用。
// 目的：采集调度、错误处理与可靠性设计（超时/重试/退避）的实现要点，修改此文件时请注意与 serial 层的并发与资源管理接口契约。
package poller
