...
此处描述了串口配置的运行时管理与持久化策略。实现要点：

- 两个 localStorage key：
  - CURRENT_SERIAL_KEY = 'ddsuv_serial_config_v1' — 当前运行时配置（由 SerialConfigPanel 在变更时写入，用于界面/运行态恢复）。
  - SAVED_SERIAL_KEY = 'ddsuv_serial_config_saved_v1' — 用户显式“保存”的配置（SettingsModal 写入，并优先通过后端 RPC 持久化）。
- 恢复约束：组件启动时从 SAVED_SERIAL_KEY 恢复到 CURRENT_SERIAL_KEY 仅在已保存配置包含有效 port 或 slaveID>0 时才写回，从而避免将无效/占位值覆盖运行态（解决启动时显示 “00” 的问题）。
- 广播：恢复成功后通过 CustomEvent('ddsuv_serial_config_restored') 广播，供页面其他组件（如表单控件）即时同步。
- 后端持久化：优先调用 Wails RPC（SaveSavedSerialConfig / LoadSavedSerialConfig / ClearSavedSerialConfig），若 RPC 不可用或失败则回退到 localStorage。
- UX：避免 window.alert/confirm，使用非阻塞 toast 与受控确认对话（SettingsModal 内实现局部 showToast 与内建确认对话作为 Chakra 缺失导出时的回退）。
...
const RESTORED_EVENT = 'ddsuv_serial_config_restored';
...
export const SerialConfigPanel = () => {
  ...
  // 启动时尝试从后端（优先）或 localStorage 恢复“已保存的配置”
  useEffect(() => {
    let loaded: any = null;

    const tryLoadFromBackend = async () => {
      try {
        // LoadSavedSerialConfig 为后端 RPC，若后端不可用会抛错
        if (window.go?.main?.App?.LoadSavedSerialConfig) {
          loaded = await window.go.main.App.LoadSavedSerialConfig();
        }
      } catch (e) {
        loaded = null;
      }
      ...
      if (loaded && hasValidPortOrSlaveID(loaded)) {
        // 仅在有效时写回 CURRENT_SERIAL_KEY，避免覆盖运行态占位值
        localStorage.setItem(CURRENT_SERIAL_KEY, JSON.stringify(loaded));
        // 广播恢复事件，detail 包含完整配置对象
        window.dispatchEvent(new CustomEvent(RESTORED_EVENT, { detail: loaded }));
      }
    };

    tryLoadFromBackend().catch(console.error);
  }, []);
  ...
  const handleConfigUpdate = async (field: string, value: any) => {
    ...
    setConfig(newConfig);
    // 将变更写入运行态 key（CURRENT_SERIAL_KEY）
    localStorage.setItem(CURRENT_SERIAL_KEY, JSON.stringify(newConfig));
    ...
    try {
      ...
      showToast(err?.message || '更新配置失败', 'error');
    }
  };

  // 工具函数：判定保存配置是否有效（至少包含 port 或 从站ID>0）
  const hasValidPortOrSlaveID = (cfg: any) => {
    if (!cfg) return false;
    if (cfg.port && cfg.port.trim()) return true;
    if (typeof cfg.slaveID === 'number' && cfg.slaveID > 0) return true;
    return false;
  };
};
```

在 UI 端，SettingsModal 提供“保存当前的串口配置”选项：
- 勾选并确认时：先写入 localStorage 的 SAVED_SERIAL_KEY，再尝试调用 window.go.main.App.SaveSavedSerialConfig(JSON.stringify(cfg))，成功时显示 success toast；若 RPC 不可用则仅保存在 localStorage 并显示 warning toast。
- 清除时：调用 ClearSavedSerialConfig（RPC）并删除 localStorage 的 SAVED_SERIAL_KEY，最终显示 toast。

该实现保持无阻塞 UX、兼容无后端运行场景，并保证启动恢复不会意外覆盖运行时有效配置。
