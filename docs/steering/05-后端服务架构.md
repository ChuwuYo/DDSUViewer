...
为了与前端的 SettingsModal / SerialConfig 恢复逻辑对齐，后端在 Service 层提供了三条用于“已保存串口配置”的 Wails RPC 接口（前端优先调用后端持久化；后端不可用则回退到 localStorage）并把配置持久化到磁盘文件。

要点：
- 持久化文件位置：data/saved_serial_config.json（相对于可执行文件工作目录）。该文件保存 JSON 格式的已保存串口配置，供应用重启后恢复。
- RPC 方法（已在 service 层实现并通过 app.go 绑定给前端）：
  - SaveSavedSerialConfig(cfgJSON string) error — 将 cfgJSON 写入 data/saved_serial_config.json（覆盖），并返回错误信息（若任何 IO 或权限问题发生）。
  - LoadSavedSerialConfig() (string, error) — 读取 data/saved_serial_config.json 并返回文件内容（JSON 字符串）；找不到或读取失败时返回空字符串与错误。
  - ClearSavedSerialConfig() error — 删除持久化文件（或清空其内容），并返回可能的错误。

示例方法签名（简化示意）：
```go
// SaveSavedSerialConfig 保存已保存的串口配置到 data/saved_serial_config.json
func (s *Service) SaveSavedSerialConfig(cfgJSON string) error

// LoadSavedSerialConfig 从磁盘加载已保存的串口配置（返回 JSON 字符串）
func (s *Service) LoadSavedSerialConfig() (string, error)

// ClearSavedSerialConfig 删除持久化文件或清空已保存配置
func (s *Service) ClearSavedSerialConfig() error
```

实现注意事项：
- IO 操作需加锁（使用 Service.mutex）以避免并发写入冲突。
- 当后端成功保存时，前端的 SettingsModal 会显示 success toast；当 RPC 不可用或返回错误时，前端应回退到将配置写入 localStorage 的 SAVED_SERIAL_KEY 并显示 warning/error toast。
- 由于前端在启动时优先尝试通过 RPC 加载已保存配置（LoadSavedSerialConfig），若 RPC 失败再读取 localStorage，从而支持开发时或无后端运行场景。
